...
# Initial Conditions
# ===================
xL, xR, N = 10e-6, 120., 3000    # limits, interval
hbar, V0 = 1.0, 4.0               # constants
m = 1.0
#m = mP                            # mass of proton
...
for L in range(Lmax):
    ...
    #u0, u1 = 0.0, (xL+h)**(L+1)  
    while (E1 < V0*np.exp(-lamda*a)/a):             # sweep E range for each L (does this condition get changed?)
        E1 += dE
...
plt.ylim(EL[L][0]-0.1, EL[L][-50]+0.5), plt.xticks(range(Lmax))
for L in range(Lmax):
    for i in range(len(EL[L])):
        plt.plot([L-.3, L+.3], [EL[L][i]]*2, 'k-', lw=0.5)
plt.show()
...
plt.xlim(0, 6)
plt.ylim(-V0-0.5, V_eff(r).max()+0.5)
plt.xlabel('$r$ (fm)')
