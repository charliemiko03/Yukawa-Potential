...
for L in range(Lmax):
    for i, psi in enumerate(list_psix[L]):
        ...
        plt.plot(r, V_eff(r), 'r--', alpha=0.5, label='$V_{eff}$')
        ...
        plt.xlim(0, 25)  # Focus on region near potential well
        plt.ylim(-22, 5)
        plt.legend()
        #plt.tight_layout()
        plt.savefig(f'wf_u{L+i+1}{L}.pdf')  # Save the figure
        plt.show()
        ...
        #plt.ylim(-22, 5)
        #plt.tight_layout()
        plt.savefig(f'pd_u{L+i+1}{L}.pdf')  # Save the figure
        plt.show()